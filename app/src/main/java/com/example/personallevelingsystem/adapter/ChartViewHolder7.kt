...
class ChartViewHolder7(itemView: View, private val mealDao: MealDao) :
    RecyclerView.ViewHolder(itemView) {
    private val title: TextView = itemView.findViewById(R.id.title7)
    private val chart: LineChart = itemView.findViewById(R.id.chart7)

    fun bind() {
        title.text = "Calories consommées"
        setupChart()
    }
    ...
    private suspend fun getLast30DaysAggregated(mealDao: MealDao): List<Pair<Long, Float>> {
        ...
        val aggregatedData = rawData.groupBy {
            ...
            val totalAmountOfCalories = entry.value.sumOf { it.calories }.toFloat()
            val date = entry.value.first().date // Any date within the same day works
            date to totalAmountOfCalories
        }.sortedBy { it.first }

        // Log the aggregated data to verify
        Log.d("AggregatedCaloriesData", "Aggregated calories data: $aggregatedData")
    ...
    private fun convertDataToEntries(dataList: List<Pair<Long, Float>>): LineData {
        ...
        val dataSet = LineDataSet(entries, "Calories consumed").apply { color = Color.WHITE}
